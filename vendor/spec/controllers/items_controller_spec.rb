...
describe ItemsController do
  ...
  describe "#index" do
    ...
    it "assigns all activated items as @items" do
      items= Item.where(:activated => true)
      get :index
      assigns(:items).should eq(items)
    end
  ...
  describe "#update" do
    ...
    xit "locates the requested item" do
      put :update, id: item, contact: item.attributes
      assigns(:item).should eq(item)
    end
    ...
    context "with valid attributes" do
      ...
      xit "changes item attributes" do
        put :update, id: item.id, item: FactoryGirl.attributes_for(:item, name: "updateditem")
        item.reload
  ...
  describe "#user_edit_item" do
    ...
    it "calls find" do
      Item.should_receive(:find_by_id)
