BestBay::Application.routes.draw do
  match 'carts/checkout' => 'carts#checkout', :as => :cart_checkout
  match 'carts/wishlist' => 'carts#wishlist', :as => :cart_wishlist
  match 'line_items/delete/:id' => 'line_items#destroy', :as => :line_items_delete
  match 'carttowishlist' => 'line_items#change_type', :as => :line_items_change_type


  match 'reports/' => 'reports#index', :as => :reports_index, :via => :get
  match 'reports/buyer_details' => 'reports#buyer_details', :as => :reports_buyer_details, :via => :get
  match 'reports/seller_details' => 'reports#seller_details', :as => :reports_seller_details, :via => :get
  match 'reports/sales_details' => 'reports#sales_details', :as => :reports_sales_details, :via => :get
  ...
  resources :auctions
  match 'users/profile/bids' => 'auctions#user_bids_index', :as => :auction_index_user , :via => :get

  match 'auctions/delete/:id' => 'auctions#destroy', :as => :auction_delete
  match 'users/profile/seller/auctions' => 'auctions#seller_auction_history_index', :as => :seller_auction_history_index , :via => :get
  match 'auctions/increase_bid' => 'auctions#increase_bid', :as => :auction_increase_bid
  match 'auctions/change_state/:id' => 'auctions#change_state', :as => :auction_change_state, :via => :post
  ...
  match 'admin/users/delete/:id' => 'admin_users#delete', :as => :admin_users_delete, :via => :delete
  ...
  match 'admin/items/delete/:id' => 'admin_items#delete', :as => :admin_items_delete, :via => :delete
  ...
  match 'admin/categories/delete/:id' => 'admin_categories#delete', :as => :admin_category_delete, :via => :delete
  match 'admin/categories/:id' => 'admin_categories#show', :as => :admin_categories, :via => :get

  match 'orders/history_orders' => 'orders#history_orders', :as => :history_orders, :via => :get
  match 'orders/track' => 'orders#track', :as => :orders_track, :via => :post

  match 'carts/confirm_checkout' => 'carts#confirm_checkout', :as => :carts_confirm_checkout, :via => :post

  match 'orders/seller_history' => 'orders#seller_history', :as => :order_seller_history, :via => :get
  match 'orders/seller_current' => 'orders#seller_current', :as => :order_seller_current , :via => :get
  match 'orders/process_order' => 'orders#process_order', :as => :order_process , :via => :get
